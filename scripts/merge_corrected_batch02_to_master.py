...
CORRECTED = Path("screening/title_abstract/working/pubmed_screening_batch_02_CORRECTED.csv")
BACKUP = Path("screening/title_abstract/pubmed_title_abstract_screening_before_batch02_merge.csv")
REPORT = Path("screening/title_abstract/pubmed_batch02_merge_report.md")
...
with REPORT.open("w", encoding="utf-8") as f:
    f.write("# PubMed Batch 02 Merge Report\n\n")
    f.write("## Merge Summary\n\n")
    f.write(f"- Corrected Batch 02 records merged: {updated}\n")
    f.write(f"- Master screening file: {MASTER}\n")
    f.write(f"- Backup created: {BACKUP}\n\n")

    f.write("## Batch 02 Decision Counts After Merge\n\n")
...
print("Batch 02 merge complete.")
print(f"Updated records: {updated}")
print("Batch 02 decisions:")
for k, v in batch_decisions.items():
    print(f"{k}: {v}")
